...
ggsave("Basic ZIP histogram.png", width=7, height=5)
```
...
ggsave("density_illustration.png", c, width=21, height=7)
...
plot_results = function(df, level=0.99) {
  ...
  mutate(term = str_replace_all(
    term, 
    c('treatment1', 'treatment2', 'treatment3'), 
    c('treatment1: π + 0.05', 'treatment2: λ + 0.5', 'treatment3: π + 0.08; λ + 0.5')
  )) %>%
  ggplot() + 
...
ggsave('recovered_coefficients_missp.png', width=15, height=5)
...
ggsave('recovered_coefficients_zipf.png', width=15, height=5)
```
...
ggsave('power_comparisons.png', width=7, height=4)
```
...
ggsave("Simulated vs. actual power compared.png", width=10, height=5)
```
...
ggsave("power_at_different_n.png", width=15, height=5)
```
